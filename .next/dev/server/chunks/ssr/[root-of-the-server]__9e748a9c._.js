...
"[project]/lib/repos/dbRepo.ts [app-ssr] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function getPrisma() {
    if (!process.env.DATABASE_URL) {
        throw new Error('DATABASE_URL not set - dbRepo unavailable');
    }
    // dynamic import so app can run without installing prisma during dev until user opts in
    try {
        // use eval to avoid static analysis by bundlers that would require resolving the package
        // eslint-disable-next-line no-eval
        const pkg = eval("require('@prisma/client')");
        const { PrismaClient } = pkg;
        return new PrismaClient();
    } catch (e) {
        throw new Error("@prisma/client is not installed or cannot be loaded");
    }
}
async function loadAll() {
    ...
    const mappedPosts = posts.map((p)=>({
            ...
            collectionIds: p.collections?.map((pc)=>pc.collectionId) ?? [],
            tags: p.tags?.map((t)=>t.tag.name) ?? [],
            savedAt: p.savedAt.toISOString(),
            editedAt: p.editedAt ? p.editedAt.toISOString() : undefined
        }));
...
async function saveAll(payload) {
    const prisma = await getPrisma();
    // naive sync: upsert categories and collections, replace posts (simple approach)
    ...
    for (const p of payload.posts){
        ...
        if (p.tags && p.tags.length) {
            for (const tName of p.tags){
                const tagId = `tag-${tName.replace(/\s+/g, '-').toLowerCase()}`;
        ...
        if (p.collectionIds && p.collectionIds.length) {
            for (const colId of p.collectionIds){
                await prisma.postCollection.upsert({
                    where: {
                        id: `${p.id}-${colId}`
                    },
                    update: {},
...
function PostsProvider({ children }) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        async function hydrate() {
            try {
                // prefer DB repo when configured
                if (("TURBOPACK compile-time value", "undefined") === 'undefined' && process.env.DATABASE_URL) {
                    // server-side environment with DB configured
                    // eslint-disable-next-line @typescript-eslint/no-var-requires
                    const { dbRepo } = __turbopack_context__.r("[project]/lib/repos/dbRepo.ts [app-ssr] (ecmascript)");
                    const saved = await dbRepo.loadAll();
                    if (saved) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$server$2f$route$2d$modules$2f$app$2d$page$2f$vendored$2f$ssr$2f$react$2e$js__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useEffect"])(()=>{
        try {
            // try to save to DB when available (server); otherwise persist to local storage
            if (("TURBOPACK compile-time value", "undefined") === 'undefined' && process.env.DATABASE_URL) {
                // eslint-disable-next-line @typescript-eslint/no-var-requires
                const { dbRepo } = __turbopack_context__.r("[project]/lib/repos/dbRepo.ts [app-ssr] (ecmascript)");
                void dbRepo.saveAll({
...
//# sourceMappingURL=%5Broot-of-the-server%5D__9e748a9c._.js.map